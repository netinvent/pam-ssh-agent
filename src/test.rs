...
pub fn set_file_permissions(filename: &Path) -> Result<()> {
    ...
    std::os::unix::fs::chown(path, Some(0), Some(0)).with_context(|| format!("Tests tried to set file {} ownership to root:root but failed", filename));
    let perms = Permissions::from_mode(0o600);
    std::fs::set_permissions(path, perms).with_context(|| format!("Tests tried to set permissions of file {} to 0o600 but failed", filename));
    Ok(())
}